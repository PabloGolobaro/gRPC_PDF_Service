...
func uploadImage(client pdfcomposer.PdfComposeClient, images []fileWithName) (io.Reader, error) {
	...
	pdf, err := getPDF(stream)
	if err != nil {
		log.Fatal(err)
	}
	return pdf, nil
}
func sendImage(stream pdfcomposer.PdfCompose_UploadImageClient, image fileWithName) error {
	req := &pdfcomposer.UploadImageRequest{
		Data: &pdfcomposer.UploadImageRequest_Info{
			Info: &pdfcomposer.ImageInfo{
				ImageType: filepath.Ext(image.name),
			},
		},
	}
	err := stream.Send(req)
	...
	reader := bufio.NewReader(image.file)
...
func getPDF(stream pdfcomposer.PdfCompose_UploadImageClient) (io.Reader, error) {
